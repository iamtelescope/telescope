...
@pytest.mark.django_db
def test_delete_kubernetes_source_without_permissions(test_user, service, kubernetes_source):
    with pytest.raises(PermissionDenied):
        service.delete(user=test_user, slug=kubernetes_source.slug)


@pytest.mark.django_db
def test_delete_kubernetes_source_with_permissions(test_user, service, kubernetes_source):
