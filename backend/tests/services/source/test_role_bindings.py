...
@pytest.mark.django_db
def test_grant_kubernetes_source_role_to_user(test_user, kubernetes_source):
    binding, created = rbac_manager.grant_source_role(
...
@pytest.mark.django_db
def test_grant_kubernetes_source_role_to_user_twice(test_user, kubernetes_source):
    # First grant
    binding1, created1 = rbac_manager.grant_source_role(
        source=kubernetes_source,
        role=SourceRole.VIEWER.value,
...
@pytest.mark.django_db
def test_grant_kubernetes_source_role_to_group(kubernetes_source):
    group = Group.objects.create(name="test_k8s_group")
...
@pytest.mark.django_db
def test_revoke_kubernetes_source_role_from_user(test_user, kubernetes_source):
    # First grant a role
    rbac_manager.grant_source_role(
        source=kubernetes_source,
        role=SourceRole.VIEWER.value,
...
@pytest.mark.django_db
def test_revoke_kubernetes_source_role_from_group(kubernetes_source):
    group = Group.objects.create(name="test_k8s_group")
...
@pytest.mark.django_db
def test_multiple_kubernetes_roles_for_same_user(test_user, kubernetes_source):
    # Grant multiple roles to the same user
    rbac_manager.grant_source_role(
        source=kubernetes_source,
        role=SourceRole.VIEWER.value,
...
@pytest.mark.django_db
def test_grant_invalid_kubernetes_source_role(test_user, kubernetes_source):
    # Try to grant an invalid role
    with pytest.raises(ValueError, match="unknown source role"):
        rbac_manager.grant_source_role(
            source=kubernetes_source,
...
@pytest.mark.django_db
def test_grant_kubernetes_source_role_without_user_or_group(kubernetes_source):
    # Try to grant a role without specifying user or group
    with pytest.raises(ValueError, match="either user or group should be provided"):
        rbac_manager.grant_source_role(
            source=kubernetes_source,
...
@pytest.mark.django_db
def test_get_kubernetes_role_bindings_with_grant_permission(kubernetes_source):
    # Create a user with OWNER role (has GRANT permission)
    owner = User.objects.create_user(username="k8s_source_owner", password="pass")
    rbac_manager.grant_source_role(
        source=kubernetes_source,
        role=SourceRole.OWNER.value,
        user=owner,
    ...
    # Create another user with VIEWER role
    viewer = User.objects.create_user(username="k8s_source_viewer", password="pass")
    rbac_manager.grant_source_role(
        source=kubernetes_source,
        role=SourceRole.VIEWER.value,
        user=viewer,
    ...
    # Owner should be able to get role bindings
    bindings = source_srv.get_role_bindings(user=owner, slug=kubernetes_source.slug)
    assert len(bindings) == 2

    # Check that both bindings are returned
    usernames = {b["user"]["username"] for b in bindings if b["user"]}
    assert "k8s_source_owner" in usernames
    assert "k8s_source_viewer" in usernames


@pytest.mark.django_db
def test_get_kubernetes_role_bindings_without_grant_permission(kubernetes_source):
    # Create a user with VIEWER role (does NOT have GRANT permission)
    viewer = User.objects.create_user(
        username="k8s_source_viewer_no_grant", password="pass"
    )
    rbac_manager.grant_source_role(
        source=kubernetes_source,
        role=SourceRole.VIEWER.value,
    ...
    # Viewer should NOT be able to get role bindings
    with pytest.raises(PermissionDenied):
        source_srv.get_role_bindings(user=viewer, slug=kubernetes_source.slug)


@pytest.mark.django_db
def test_get_kubernetes_role_bindings_with_editor_role(kubernetes_source):
    # Create a user with EDITOR role (does NOT have GRANT permission)
    editor = User.objects.create_user(
        username="k8s_source_editor_no_grant", password="pass"
    )
    rbac_manager.grant_source_role(
        source=kubernetes_source,
        role=SourceRole.EDITOR.value,
    ...
    # Editor should NOT be able to get role bindings (only OWNER has GRANT)
    with pytest.raises(PermissionDenied):
        source_srv.get_role_bindings(user=editor, slug=kubernetes_source.slug)
...
def test_get_kubernetes_role_bindings_with_user_role(kubernetes_source):
    ...
    # USER role should NOT be able to get role bindings
    with pytest.raises(PermissionDenied):
        source_srv.get_role_bindings(user=user, slug=kubernetes_source.slug)


@pytest.mark.django_db
def test_get_kubernetes_role_bindings_with_raw_query_user_role(kubernetes_source):
    # Create a user with RAW_QUERY_USER role (does NOT have GRANT permission)
    raw_query_user = User.objects.create_user(
        username="k8s_source_raw_query", password="pass"
    )
    rbac_manager.grant_source_role(
        source=kubernetes_source,
        role=SourceRole.RAW_QUERY_USER.value,
    ...
    # RAW_QUERY_USER should NOT be able to get role bindings
    with pytest.raises(PermissionDenied):
        source_srv.get_role_bindings(user=raw_query_user, slug=kubernetes_source.slug)


@pytest.mark.django_db
def test_get_kubernetes_role_bindings_no_access_at_all(kubernetes_source):
    # Create a user with no roles on this source
    no_access_user = User.objects.create_user(
        username="k8s_source_no_access", password="pass"
    )

    # User with no access should NOT be able to get role bindings
    with pytest.raises(PermissionDenied):
        source_srv.get_role_bindings(user=no_access_user, slug=kubernetes_source.slug)
