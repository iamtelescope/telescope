<template>
  <div class="flex flex-col overflow-y-auto">
    <div class="text-2xl p-4 border-b border-neutral-300 dark:border-neutral-600"><i class="pi pi-eye"/> <span
        class="font-medium">{{ source.name }}</span> source
      Saved Views
    </div>
    <DataView :loadings="[loading]" :errors="[error]">
      <div class="p-4 w-full" v-if="savedViewsPrepared.length > 0">
        <IconField>
          ...
          <InputText placeholder="Filter saved views" v-model="savedViewFilter" :autofocus="true" class="w-full"/>
        </IconField>
      </div>
      <DataTable
            ...
            <div class="flex flex-col">
              <div class="flex flex-row justify-between w-full">
                <div class="text-nowrap">
                  <span class="text-primary" :class="{'font-bold': currentView?.slug === data.slug}"
                        v-tooltip.left="data.name.length > 64 ? data.name : ''">{{
                      getShortedValue(data.name)
                    }}</span>
                </div>
              ...
              <div class="text-gray-500 dark:text-gray-400 text-sm" v-if="data.description">{{ data.description }}</div>
              <div class="flex flex-col pt-2" v-if="data.data.fields || data.data.query">
                <div class="flex flex-row items-center text-sm  text-gray-500 dark:text-gray-400"
                     v-if="data.data.fields">
                  <i class="pi pi-list pr-2"/> <span
                    v-tooltip.left="data.data.fields.length > 64 ? data.data.fields : ''">{{
                    getShortedValue(data.data.fields)
                  }}</span>
                </div>
                <div class=" flex flex-row items-center text-sm text-gray-500 dark:text-gray-400"
                     v-if="data.data.query">
                  <i class="pi pi-search pr-2"/> <span
                    v-tooltip.left="data.data.query.length > 64 ? data.data.query : ''">{{
...
const {
  ...
  error,
  loading,
} = useGetSavedViews(props.source.slug)
