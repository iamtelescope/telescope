<template>
  <div>
    <Drawer v-model:visible="savedViewsListVisible" :modal="false" position="right" pt:root:class="w-[650px]">
      <template #container>
        <SavedViewsList :source="source" :view="viewToUpdate" :currentView="currentView" @change="onSavedViewChange"/>
      </template>
    </Drawer>
    <Dialog class="w-[600px] h-[600px]" v-model:visible="savedViewFormVisible" :modal="true" draggable>
      <template #container="{ closeCallback }">
        <SavedViewForm :source="source" :view="viewToUpdate" @close="closeCallback" @change="onSavedViewChange"/>
      </template>
    </Dialog>
    <Dialog class="w-[400px]" v-model:visible="savedViewDeleteDialogVisible" :modal="true"
            draggable :closable="savedViewDeleteDialogClosable" :closeOnEscape="savedViewDeleteDialogClosable">
      <template #container="{ closeCallback }">
        <SavedViewDeleteDialog :source="source" :view="viewToUpdate" @close="closeCallback"
                               @deleted="onSavedViewDelete" @actionStarted="savedViewDeleteDialogClosable = false"
    ...
    <div class="flex flex-row items-center">
      <div class="flex flex-row items-center mr-2 bg-blue-50 dark:bg-gray-800 pt-2 pb-2 pr-3 pl-3 rounded-lg">
        <span class="font-medium text-sm"
            ...
            currentView?.name?.length > 48 ? currentView.name.slice(0, 45) + '…' : currentView?.name || 'Default view'
          }}</span>
      </div>
      <Button icon="pi pi-ellipsis-v" size="small" text @click="toggleSavedViewsMenu"
