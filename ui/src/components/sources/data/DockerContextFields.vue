<template>
    <FloatLabel variant="on">
        <MultiSelect id="container" v-model="selectedContainers" :initialValues="initialValues" optionLabel="name"
            display="chip" :options="data" :loading="loading" class="mr-2 min-w-80" :maxSelectedLabels="5" filter
            autoFilterFocus @change="onContainersChange">
            <template #option="slotProps">
                <div class="flex flex-col">
                    <div class="flex flex-fow items-center">
                        <i class="pi pi-circle-fill text-gray-300"
                            :class="{ 'text-green-500': slotProps.option.status == 'running', 'text-red-500': slotProps.option.status == 'dead', }"></i>
